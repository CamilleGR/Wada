...
histoB.style.visibility="hidden";
 


 d3.csv("data.csv", function (data) { //   --------------------------	NOM OU CHEMIN DU CSV ------------------------------- // 
...
document.getElementById("max").innerHTML =  d3.max(data, function(d) { return d.value; }); 
document.getElementById("min").innerHTML =  d3.min(data, function(d) { return d.value; }); 
...
donutB.style.visibility="hidden";
...
courbeB.style.visibility="hidden";
        var margin =  {top: 20, right: 10, bottom: 20, left: 40},
            selectorHeight = 20
            width = 1000 - margin.left - margin.right,
...
var	parselabel = d3.time.format("%d-%b").parse;
